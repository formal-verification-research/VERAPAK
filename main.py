...
def setup(config):
    ...
    config["Point"] = np.fromstring(config["Point"],
            dtype=config["Graph"].input_dtype, sep=",")
    .reshape(config["Graph"].input_shape)
    
    if config["Label"] is None:
        config["Label"] = np.argmax(config["Graph"].evaluate(config["Point"])) 

    if len(config["Granularity"]) == 1:
        config["Granularity"] = np.full_like(config["Point"], config["Granularity"][0])
    else:
        config["Granularity"] = config["Granularity"].reshape(config["Point"].shape)
        .astype(config["Graph"].input_dtype)

    if len(config["Radius"]) == 1:
        config["Radius"] = np.full_like(config["Point"], config["Radius"][0])
    else:
        config["Radius"] = config["Radius"].reshape(config["Point"].shape)
        .astype(config["Graph"].input_dtype)
