...
VNNLIB=$4
RESULTS_FILE=$5
TIMEOUT=$(echo "$6/60" | bc)

echo $TIMEOUT

docker run --rm -it -v $PWD/vnncomptests:/src/vnncomptests -v $PWD/out:/src/out verapak:latest python main.py --config_file=vnncomptests/vnncomp.conf --output_dir=out --timeout_minutes=$TIMEOUT
