...
def update_rv_tuple(rv_tuple, op, first, second, num_inputs, num_outputs):
    ...
    if first.startswith("X_"):
        ...
        assert not second.startswith("X") and not second.startswith("Y"), \
                                     "input constraints must be box (" + op + " " + first + " " + second + ")"
        ...
        assert limits[0] <= limits[1], first + " range is empty: " + limits
...
def read_vnnlib_simple(vnnlib_filename, num_inputs, num_outputs):
    ...
    for line in lines:
        ...
        if groups:
            assert len(groups[0]) == 3, "groups was " + groups + ": " + line
        ...
        assert groups, "failed parsing line: " + line
    ...
    for rv_tuple in merged_rv.values():
        ...
        for d in range(num_inputs):
            ...
            assert r[0] != -np.inf and r[1] != np.inf, "input X_" + d + " was unbounded: " + r
            box.append(r)
...
if __name__ == "__main__":
    import sys
    try:
        sys.argv.index("-codeout")
        sys.argv.remove("-codeout")
        codeout = True
    except Exception:
        codeout = False
    try:
    ...
    if len(sys.argv) < 2:
        print("Usage: " + sys.argv[0] + " [-codeout] [-v] <f1.vnnlib> [f2.vnnlib [...]]")
        print("       -codeout: Use exit codes to define type (1 = None, 2 = Minimal, 3 = Maximal, 0 = ??)")
        print("                 Works only when one file is provided")
        ...
        if len(sys.argv) > 2:
            ...
            num_succ = 0
            num_fail = 0
            sys.stderr.write("Reading " + str(len(sys.argv) - 1) + " vnnlib files\n")
            succ = []
            fail = []
            for f in sys.argv[1:]:
                verbose_print("\n --- " + f + " --- ", verbose)
                vars_in, vars_out = count_variables(f)
                v = encode(read_vnnlib_simple(f, vars_in, vars_out))
                verbose_print("IN : " + str(v.inputs), verbose)
                num_done += 1
                if can_process(v):
                    num_succ += 1
                    verbose_print(repr(v))
                    succ.append(f)
                else:
                    num_fail += 1
                    fail.append(f)
                sys.stderr.write("\r" + str(num_done) + " / " + num + " (+: " + str(num_succ) + ", -: " + str(num_fail) + ")")
            verbose_print("\n\n", verbose)
            if len(succ) > 0:
                print("\n == SUCCESS == ")
                for f in succ:
            ...
            vars_in, vars_out = count_variables(sys.argv[1])
            verbose_print(str(vars_in) + " " + str(vars_out), verbose)
            v = encode(read_vnnlib_simple(sys.argv[1], vars_in, vars_out))
            print("IN : " + str(v.inputs))
            vtype = get_type(v, verbose=verbose)
            if vtype in [1,2]:
                verbose_print(repr(v), verbose)
                verbose_print("\n == SUCCESS == ", (verbose and codeout) or not codeout)
                if codeout:
                    sys.exit(vtype)
                sys.exit(0)
            else:
                verbose_print("\n == FAILURE == ", (verbose and codeout) or not codeout)
                if codeout:
                    sys.exit(vtype)
                sys.exit(1)
